...
class NoiseClient:
    ...
    def perform_handshake(self):
        ...
        try:
            # Start handshake
            initial_message = self.noise_handler.handshake_step(None)
            
            if initial_message is None:
                logger.error("Failed to generate initial handshake message")
                return False
            
            logger.info(f"Client sending initial message: {len(initial_message)} bytes")
            
            # Send the message with length prefix as a single operation
            message_length = struct.pack('!I', len(initial_message))
            self.client_socket.sendall(message_length + initial_message)
            ...
            server_message = self.client_socket.recv(message_length)
            
            if len(server_message) != message_length:
                logger.error("Incomplete handshake message from server")
                return False
            
            logger.info(f"Client received server message: {len(server_message)} bytes")
            
            # Process server's response and complete handshake
            self.noise_handler.handshake_step(server_message)
